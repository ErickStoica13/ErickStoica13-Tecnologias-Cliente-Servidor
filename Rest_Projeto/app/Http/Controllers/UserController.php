    ...
    public function editarCandidato(Request $request)
{
    ...
    try {
        ...
        if ($request->has('experiencia')) {
            ...
            $user->experiencia()->delete(); 
            $user->experiencia()->createMany($experiencias); //criando nova experiencia
        }
...
/*
public function lerCompetenciaPropria(Request $request)
{
    $user = $request->user();
    ...
    $competencias = $user->competencias()->get(['id', 'nome']);
...
{
    $validator = Validator::make($request->all(), [
        'id' => 'required|integer|exists:competencias,id',
        'nome' => 'required|string',
    ], [
        'id.required' => 'O campo ID é obrigatório.',
        'id.integer' => 'O campo ID deve ser um número inteiro.',
        'id.exists' => 'Competência não encontrada.',
        'nome.required' => 'O campo nome é obrigatório.',
    ]);
    ...
    $competencia = Competencia::find($request->input('id'));
    $competencia->nome = $request->input('nome');
    $competencia->save();

    return response()->json(['mensagem' => 'Competência editada com sucesso'], 200);
}

public function deletarCompetencia(Request $request)
{
    $validator = Validator::make($request->all(), [
        'id' => 'required|integer|exists:competencias,id',
    ], [
    ...
    return response()->json(['mensagem' => 'Competência deletada com sucesso'], 200);
}

public function cadastroExperiencia(Request $request)
{
    $validator = Validator::make($request->all(), [
        'id' => 'required|integer|unique:experiencias,id',
        ...
        'fim.date' => 'O campo data de fim deve ser uma data válida.',
    ...
    return response()->json(['mensagem' => 'Experiência cadastrada com sucesso'], 201);
}

public function lerExperiencias(Request $request)
{
    $user = $request->user();
    ...
    $experiencias = $user->experiencia()->get(['id', 'nome_empresa', 'inicio', 'fim', 'cargo']);
...
{
    $validator = Validator::make($request->all(), [
        'id' => 'required|integer|exists:experiencias,id',
        'nome_empresa' => 'required|string',
        'inicio' => 'required|date',
        'fim' => 'nullable|date',
        'cargo' => 'required|string',
        ...
        'id.required' => 'O campo ID é obrigatório.',
        'id.integer' => 'O campo ID deve ser um número inteiro.',
        'id.exists' => 'Experiência não encontrada.',
        'nome_empresa.required' => 'O campo nome da empresa é obrigatório.',
        'inicio.required' => 'O campo data de início é obrigatório.',
        'inicio.date' => 'O campo data de início deve ser uma data válida.',
        'fim.date' => 'O campo data de fim deve ser uma data válida.',
    ...
    $experiencia = Experiencia::find($request->input('id'));
    $experiencia->update([
        'nome_empresa' => $request->input('nome_empresa'),
        'inicio' => $request->input('inicio'),
        'fim' => $request->input('fim'),
    ...
    return response()->json(['mensagem' => 'Experiência editada com sucesso'], 200);
}
...
public function deletarExperiencia(Request $request)
{
    ...
    return response()->json(['mensagem' => 'Experiência deletada com sucesso'], 200);
}
